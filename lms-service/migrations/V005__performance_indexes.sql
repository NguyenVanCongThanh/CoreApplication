...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_student_answers_attempt_grading
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_student_answers_ungraded
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_content_progress_content_student
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_content_progress_student_course
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_quiz_attempts_analytics
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_quiz_attempts_student_status
    ON quiz_attempts(student_id, quiz_id, status, submitted_at)
    WHERE status IN ('SUBMITTED', 'GRADED');

-- ── flashcard_repetitions ─────────────────────────────────────────────────────
-- Used by: ListDueFlashcards, GetFlashcardStats
-- Pattern: find cards due today for a student in a course
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_flashcard_rep_due_today
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_enrollments_course_accepted
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_section_content_section_mandatory
...
-- Note: primary key index already covers single-row lookups.
--       This partial index helps the ANY($1) array scan on submitted quizzes.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_forum_posts_score
