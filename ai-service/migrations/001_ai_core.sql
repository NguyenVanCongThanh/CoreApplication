-- =============================================================
-- AI Service — isolated database schema
-- External LMS entity IDs (course_id, student_id, content_id …)
-- are stored as plain BIGINT. Referential integrity for cross-DB
-- references is enforced at application layer, not DB layer.
-- Internal AI-domain FK constraints are kept as normal.
-- =============================================================
...
CREATE TABLE IF NOT EXISTS knowledge_nodes (
    ...
    course_id             BIGINT NOT NULL,        -- soft ref → lms.courses.id
    parent_id             BIGINT REFERENCES knowledge_nodes(id) ON DELETE SET NULL,
    name                  VARCHAR(255) NOT NULL,
    name_vi               VARCHAR(255),
    name_en               VARCHAR(255),
    description           TEXT,
    description_embedding VECTOR(1024),           -- bge-m3 1024d
    level                 INTEGER DEFAULT 0,
    order_index           INTEGER DEFAULT 0,
    source_content_id     BIGINT,                 -- soft ref → lms.section_content.id
    auto_generated        BOOLEAN DEFAULT false,
    created_at            TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at            TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_kn_course  ON knowledge_nodes(course_id);
CREATE INDEX idx_kn_parent  ON knowledge_nodes(parent_id);
CREATE INDEX idx_kn_level   ON knowledge_nodes(course_id, level);
CREATE INDEX idx_kn_source  ON knowledge_nodes(source_content_id)
    WHERE source_content_id IS NOT NULL;
CREATE INDEX idx_kn_emb_hnsw ON knowledge_nodes
...
-- =============================================================
-- KNOWLEDGE NODE RELATIONS  (graph edges)
-- =============================================================
...
CREATE INDEX idx_knr_source ON knowledge_node_relations(source_node_id);
CREATE INDEX idx_knr_target ON knowledge_node_relations(target_node_id);
CREATE INDEX idx_knr_course ON knowledge_node_relations(course_id);
...
CREATE TABLE IF NOT EXISTS document_chunks (
    ...
    content_id      BIGINT,                      -- soft ref → lms.section_content.id
    course_id       BIGINT NOT NULL,             -- soft ref → lms.courses.id
...
CREATE INDEX idx_dc_emb_hnsw ON document_chunks
    USING hnsw (embedding vector_cosine_ops)
    WITH (m = 16, ef_construction = 128);

CREATE INDEX idx_dc_content ON document_chunks(content_id);
CREATE INDEX idx_dc_node    ON document_chunks(node_id);
CREATE INDEX idx_dc_course  ON document_chunks(course_id);
CREATE INDEX idx_dc_status  ON document_chunks(status);
CREATE INDEX idx_dc_hash    ON document_chunks(chunk_hash);
...
CREATE TABLE IF NOT EXISTS document_processing_jobs (
    id             BIGSERIAL PRIMARY KEY,
    content_id     BIGINT NOT NULL,              -- soft ref → lms.section_content.id
    course_id      BIGINT NOT NULL,              -- soft ref → lms.courses.id
    node_id        BIGINT REFERENCES knowledge_nodes(id) ON DELETE SET NULL,
...
CREATE INDEX idx_dpj_content ON document_processing_jobs(content_id);
...
-- =============================================================
-- AI DIAGNOSES  (Phase 1 — error diagnosis results)
-- =============================================================

CREATE TABLE IF NOT EXISTS ai_diagnoses (
    id              BIGSERIAL PRIMARY KEY,
    student_id      BIGINT NOT NULL,             -- soft ref → lms.users.id
    attempt_id      BIGINT,                      -- soft ref → lms.quiz_attempts.id
    question_id     BIGINT,                      -- soft ref → lms.quiz_questions.id
    node_id         BIGINT REFERENCES knowledge_nodes(id) ON DELETE SET NULL,
    wrong_answer    TEXT,
    correct_answer  TEXT,
...
CREATE INDEX idx_ad_student  ON ai_diagnoses(student_id);
CREATE INDEX idx_ad_question ON ai_diagnoses(question_id);
CREATE INDEX idx_ad_node     ON ai_diagnoses(node_id);

-- =============================================================
-- STUDENT KNOWLEDGE PROGRESS  (heatmap backing table)
-- =============================================================
...
-- =============================================================
-- SPACED REPETITIONS  (SM-2 algorithm state)
-- =============================================================
...
CREATE TABLE IF NOT EXISTS spaced_repetitions (
    ...
    quality_last     INTEGER DEFAULT 0,          -- 0–5
...
CREATE INDEX idx_sr_due    ON spaced_repetitions(student_id, next_review_date);
...
-- =============================================================
-- AI QUIZ GENERATIONS  (Phase 2 — DRAFT questions awaiting review)
-- =============================================================
...
CREATE TABLE IF NOT EXISTS ai_quiz_generations (
    ...
    node_id          BIGINT REFERENCES knowledge_nodes(id) ON DELETE CASCADE,
    course_id        BIGINT NOT NULL,            -- soft ref → lms.courses.id
    created_by       BIGINT NOT NULL,            -- soft ref → lms.users.id
    ...
    review_note      TEXT,
    reviewed_by      BIGINT,                     -- soft ref → lms.users.id
    reviewed_at      TIMESTAMP,
    quiz_question_id BIGINT,                     -- soft ref → lms.quiz_questions.id
    created_at       TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at       TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE INDEX idx_fc_student_node ON flashcards(student_id, node_id);
CREATE INDEX idx_fc_course       ON flashcards(course_id);
...
-- =============================================================
-- FLASHCARD REPETITIONS  (SM-2 state per flashcard)
-- =============================================================
...
CREATE INDEX idx_fcr_due ON flashcard_repetitions(student_id, course_id, next_review_date);
...
-- =============================================================
-- EMBEDDING REINDEX JOBS  (bge-m3 migration tracking)
-- =============================================================

CREATE TABLE IF NOT EXISTS embedding_reindex_jobs (
    id            BIGSERIAL PRIMARY KEY,
    course_id     BIGINT,                        -- soft ref → lms.courses.id
    content_id    BIGINT,                        -- soft ref → lms.section_content.id
...
CREATE INDEX idx_erj_status  ON embedding_reindex_jobs(status);
CREATE INDEX idx_erj_course  ON embedding_reindex_jobs(course_id);
CREATE INDEX idx_erj_content ON embedding_reindex_jobs(content_id);
...
-- =============================================================
-- VIEW: reindex progress dashboard
-- =============================================================
